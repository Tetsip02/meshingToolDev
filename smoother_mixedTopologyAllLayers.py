...
with open("./dataOutput/V2_neighbours_and_diagonals.txt", "rb") as fp:   # Unpickling
...
for layer in range(nLayers):
    ...
    # control functions
    if layer == 0:
        xiCF, etaCF, xi_xiCF, eta_etaCF, xi_etaCF, zetaCF, zeta_zetaCF = get_derivatives(level1, level1_normals, level2_normals, layerHeight[layer], layerHeight[layer + 1], directValence, directNeighbours, neighbours_and_diagonals)
        zetaCF = level1_normals
        zeta_zetaCF *= 0
    else:
        xiCF, etaCF, xi_xiCF, eta_etaCF, xi_etaCF, zetaCF, zeta_zetaCF = get_derivatives(level1, theta_input, level1_normals, layerHeight[layer - 1], layerHeight[layer], directValence, directNeighbours, neighbours_and_diagonals)
    ...
    level1 = level2
    theta_input = level1_normals.copy() #not sure again if normalized or not
...
meshio.write("./output/mixedT_completeMesh.vtk", mesh, file_format="vtk", binary=False)
