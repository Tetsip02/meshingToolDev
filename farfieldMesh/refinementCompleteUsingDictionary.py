...
mesh = { 0: {'cells': z_hexas, 'targets': [], 'cut_cells': [], 'target_index': [], 'siblings': [], 'ID': rootIndex[: , None], 'midpoints': []}}
...
while True:
    ...
    for cell in cut_cell_tmp:
        cell_neigh = all_root_neighbours3(cell)
...
mesh[1] = {'cells': [], 'targets': [], 'cut_cells': [], 'target_index': [], 'siblings': [], 'ID': [], 'midpoints': []}
next_level_mesh = []
...
mesh[1]['cells'] = np.array(next_level_mesh)

# mesh[1] = {'cells': next_level_mesh, 'targets': [], 'cut_cells': [], 'target_index': [], 'siblings': [], 'ID': [], 'midpoints': []}
# print(mesh[0]['targets'])
# print(mesh[1]['ID'])
# to_search = list(set(mesh[level]['cut_cells']).intersection(mesh[level]['targets']))
# to search = list of root cells marked for both refinement and intersection
level += 1 # level = 1
# children_template = np.array([0, 1, 2, 3, 4, 5, 6, 7])
# instead of cehcking every child of every level0 cut_cell, find start_cell first, then get neighbours, if neighbour parent not in level0:cutcells, remove neighbour from list, check for intersection, if true get neighbours, check for intersection, etc.
# for root in to_search:
#     first_octant = mesh[level - 1]['targets'].index(root) * 8
#     for i in range(8):
#         cell = first_octant + i
#         if intersects_surface(cell):
#             mesh[level]['cut_cells'].append(cell)
current_mesh = mesh[level]['cells']
midpoints = all_points[current_mesh[:, 0]] + maxCellSize * 0.5 ** (level + 1)
mesh[level]['midpoints'] = midpoints
distToMidpoint = np.sum((surface_points[0] - midpoints) ** 2, axis = 1)
start_cell = np.argmin(distToMidpoint)
mesh[level]["cut_cells"].append(start_cell)
neighbours_tmp = all_siblings(start_cell, level)
mesh[level]["target_index"].append(start_cell)
mesh[level]["siblings"].append(neighbours_tmp)
while True:
    cut_cell_tmp = []
    ...
    neighbours_tmp = []
    for cell in cut_cell_tmp:
        cell_neigh = all_siblings(cell, level)
        mesh[level]["target_index"].append(cell)
        mesh[level]["siblings"].append(cell_neigh)
        neighbours_tmp.extend(cell_neigh)
        # store neighbours
...
mesh[2] = {'cells': [], 'targets': [], 'cut_cells': [], 'target_index': [], 'siblings': [], 'ID': [], 'midpoints': []}
next_level_mesh = []
for target in mesh[level]["targets"]:
    # split level 0 target cell into 8 children and add children to level1_mesh:
    cell = current_mesh[target]
    ...
    chidlren_coords = newPointsTemplate * 0.5 ** (level + 1) * maxCellSize + all_points[cell[0]]
    all_points = np.concatenate((all_points, chidlren_coords), axis = 0)
mesh[2]['cells'] = np.array(next_level_mesh)

level += 1 # level = 2
print(mesh[2]['ID'])
testCell = 0
x = all_siblings(testCell, level)
...
##############################################
# assemble mesh
# refined_mesh = np.concatenate((np.delete(mesh[0]["cells"], mesh[0]["targets"], axis = 0), np.delete(mesh[1]["cells"], mesh[1]["targets"], axis = 0), mesh[2]["cells"]), axis = 0)
